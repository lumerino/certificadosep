...
public function cargaAtt(&$nodo, $attr) {
	...
	function xml_sella($arr) {
		...
		$certificadom=$this->getCertificate($_SERVER['DOCUMENT_ROOT']."/managecesba/cer/".$certificado, false );
			$certificadofisico=$this->generateCertificate($_SERVER['DOCUMENT_ROOT']."/managecesba/cer/".$certificado);
		...
		$this->root->setAttribute("certificadoResponsable",$certificadom);
...
// $public_key = openssl_pkey_get_public(file_get_contents($_SERVER['DOCUMENT_ROOT']."/managecesba/tempcer.cer.pem"));
// $fp = fopen($_SERVER['DOCUMENT_ROOT']."/managecesba/tempcer.cer.pem", "r");
// $cert = fread($fp, 8192);
// fclose($fp);
// $result = openssl_verify(trim($cadena_original), $Firma,$public_key, "sha256WithRSAEncryption");
...
return $sello;
		...
		function gen_cadena_original($arr)
		{
			$cadena="|";
			$cadena.="|".$arr['version'];
	...
	for($i=0;$i<count($arr['Asignaturas']["Asignatura"]);$i++){
		...
		if ( openssl_sign( utf8_encode($data), $cryptedata,  $pkeyid, OPENSSL_ALGO_SHA256) ) {
		//if ( openssl_sign( $data, $cryptedata,  $pkeyid, "sha256WithRSAEncryption") ) {
	...
	//		return $cryptedata;
		...
		function generateCertificate($cer_path){
		...
		function generatePrivateKey($key_path,$password){
		...
		function getPFX ( $cer_path,$key_path,$password )
		{
		...
		function getPrivateKey ( $key_path, $password )
		{
